...
class WildLifePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final List<WildLifeSight> sights =
        ref.watch(patrolLogProvider).wildlifeSights;
    if (sights.isEmpty) {
      for (int i = 0; i < wildLifeDefaultItems.length; i++) {
        WildLifeSight currentSight = WildLifeSight(
            name: wildLifeDefaultItems[i],
            amount: 0,
    ...
    sightControllers = sights
        .map((sight) => SightController(
            sight: sight,
            controller: TextEditingController(text: sight.amount.toString())))
        .toList();
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (BuildContext context, int index) {
                  return ListTile(
                      ...
                      leading: Image(
                          image: ResizeImage(
                              AssetImage(sightControllers[index]
                                      .sight
                                      .localImagePath ??
                                  ''),
                              width: 200,
        ...
        FilledButton(
            onPressed: () {
              for (int i = 0; i < sightControllers.length; i++) {
                ref.read(patrolLogProvider.notifier).addOrUpdateWildLifeSight(
                    WildLifeSight(
                        name: sightControllers[i].sight.name,
                        amount: int.parse(sightControllers[i].controller.text),
                        localImagePath: sights[i].localImagePath,
                        createdBy: Vault.userName ?? "-"));
              }
